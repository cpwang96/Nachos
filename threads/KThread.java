...
public class KThread {
	...
	public static void finish() {
		...
		// if the about-to-finished thread has an entry in the map,
		// it means there was a join() called and the parent needs to be
		// waken up.
		if (parentMap.containsKey(currentThread)) {
			// get the parent of this thread
			KThread parent = parentMap.get(currentThread);
			// put the parent on the readyQueue
			parent.ready();
		}
		sleep();
	}
	...
	public void join() {
		...
		Lib.assertTrue(this != currentThread);
		// assert if it's a second time call on join();
		// if it's a first time call, no key for child thread
		Lib.assertTrue(parentMap.get(this) == null);
		// if the child thread is finished, just return
		if (this.status == statusFinished) {
			return;
		}

		Machine.interrupt().disable();
		// create a mapping between child to parent
		parentMap.put(this, currentThread);
		// sleep the parent thread, wait for the child to run
		sleep();
		Machine.interrupt().enable();
	}
	...
	// HashMap used in join(), key: child thread, value: parent thread
	private static HashMap<KThread, KThread> parentMap = new HashMap<>();
